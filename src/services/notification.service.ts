...
class NotificationService {
  private getTimeUntilExpiry(expiryDate: Date): string {
    const hours = Math.ceil((expiryDate.getTime() - new Date().getTime()) / (1000 * 60 * 60));
  ...
  private async sendExpiryEmail(notification: ISubscriptionNotification): Promise<boolean> {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
        ...
        <p>Your subscription <strong>${notification.subscriptionName}</strong> ${timeUntilExpiry}.</p>
  ...
  private async sendExpirySMS(notification: ISubscriptionNotification): Promise<boolean> {
    if (!notification.userPhone) return false;

    const timeUntilExpiry = this.getTimeUntilExpiry(notification.expiryDate);
    const message = `Hi ${notification.userName}, your subscription "${notification.subscriptionName}" ${timeUntilExpiry}. Expiry: ${notification.expiryDate.toLocaleString()}. Please renew to continue services.`;

    return smsService.sendSMS(notification.userPhone, message);
  }

  async sendSubscriptionNotification(notification: ISubscriptionNotification): Promise<void> {
    const logContext = {
    ...
    try {
      const results = await Promise.allSettled([
        notification.userEmail ? this.sendExpiryEmail(notification) : Promise.resolve(false),
        notification.userPhone ? this.sendExpirySMS(notification) : Promise.resolve(false)
      ]);
      ...
      // Log failures
      if (emailResult.status === 'rejected' || (emailResult.status === 'fulfilled' && !emailResult.value)) {
        logger.error('Failed to send email notification', logContext);
      }
      if (smsResult.status === 'rejected' || (smsResult.status === 'fulfilled' && !smsResult.value)) {
  ...
  async processSubscriptionNotifications(notifications: ISubscriptionNotification[]): Promise<void> {
    // Process each notification independently
    await Promise.all(
      notifications.map(notification => this.sendSubscriptionNotification(notification))
    );
  }
}
