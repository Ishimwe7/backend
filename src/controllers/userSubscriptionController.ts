...
class UserSubscriptionController {
  ...
  async create(req: AuthRequest, res: Response):Promise<void> {
    ...
    try {
      ...
      const subscriptionExists = await Subscription.findById(req.body.subscription_id);
      ...
      res.status(400).json({ message: 'Error creating user subscription', error });
    }
  }
  ...
  async getUserSubscriptions(req: AuthRequest, res: Response) {
    ...
    try {
      const userSubscriptions = await UserSubscription.find({ user_id: user_id })
        .populate('subscription')
        .sort({ end_date: -1 });
      res.status(200).json(userSubscriptions);
    } catch (error) {
      res.status(500).json({ message: 'Error fetching user subscriptions', error });
    }
  }

  // Get active subscriptions for a user
  async getActiveUserSubscriptions(req: AuthRequest, res: Response):Promise<void> {
    try {
      ...
      res.status(500).json({ message: 'Error fetching active user subscriptions', error });
    }
  }
  ...
  async cleanExpiredSubscriptions(req: AuthRequest, res: Response):Promise<void> {
    ...
    try {
      ...
      const expiredSubscriptionIds = expiredSubscriptions.map(sub => sub.subscription);
      ...
      await User.findByIdAndUpdate(
        ...
        { 
          ...
          active_subscription: activeSubscriptions.length > 0 ? 
            activeSubscriptions[0].subscription : null
        }
      ).session(session);
      ...
      res.status(500).json({ message: 'Error cleaning expired subscriptions', error });
    } finally {
      session.endSession();
    }
  }
  ...
  async checkActiveSubscription(req: AuthRequest, res: Response):Promise<void> {
    try {
      const currentDate = new Date();
